 ...
 * Page class
 */
class RequestNoteEdit extends RequestNote
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RequestNoteEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("request_note_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->request_note_id->setQueryStringValue($keyValue);
                $this->request_note_id->setOldValue($this->request_note_id->QueryStringValue);
            } elseif (Post("request_note_id") !== null) {
                $this->request_note_id->setFormValue(Post("request_note_id"));
                $this->request_note_id->setOldValue($this->request_note_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("request_note_id") ?? Route("request_note_id")) !== null) {
                    $this->request_note_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("RequestNoteList"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "RequestNoteList") {
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("RequestNoteList"), "", $this->TableVar, true);
        $pageId = "edit";
