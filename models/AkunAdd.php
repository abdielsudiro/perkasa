...
class AkunAdd extends Akun
{
    ...
    public function run()
    {
        ...
        $this->isSubAccount->setVisibility();
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("AkunList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "AkunList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "AkunView") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->isSubAccount->CurrentValue = null;
        $this->isSubAccount->OldValue = $this->isSubAccount->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'isSubAccount' first before field var 'x_isSubAccount'
        $val = $CurrentForm->hasValue("isSubAccount") ? $CurrentForm->getValue("isSubAccount") : $CurrentForm->getValue("x_isSubAccount");
        if (!$this->isSubAccount->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->isSubAccount->Visible = false; // Disable update for API request
            } else {
                $this->isSubAccount->setFormValue($val);
            }
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        $this->isSubAccount->CurrentValue = $this->isSubAccount->FormValue;
    }
    ...
    protected function newRow()
    {
        ...
        $row['isSubAccount'] = $this->isSubAccount->CurrentValue;
    ...
    protected function validateForm()
    {
        ...
        if ($this->isSubAccount->Required) {
            if ($this->isSubAccount->FormValue == "") {
                $this->isSubAccount->addErrorMessage(str_replace("%s", $this->isSubAccount->caption(), $this->isSubAccount->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // uraian
        $this->uraian->setDbValueDef($rsnew, $this->uraian->CurrentValue, "", false);

        // parent_id
        $this->parent_id->setDbValueDef($rsnew, $this->parent_id->CurrentValue, null, false);

        // isSubAccount
        $tmpBool = $this->isSubAccount->CurrentValue;
        if ($tmpBool != "1" && $tmpBool != "0") {
            $tmpBool = !empty($tmpBool) ? "1" : "0";
        }
        $this->isSubAccount->setDbValueDef($rsnew, $tmpBool, null, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("AkunList"), "", $this->TableVar, true);
