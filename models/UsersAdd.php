...
class UsersAdd extends Users
{
    ...
    public function run()
    {
        ...
        $this->user_level_permission_id->setVisibility();
        $this->_username->setVisibility();
        $this->_password->setVisibility();
        $this->nama_lengkap->setVisibility();
        $this->satker->setVisibility();
        ...
        // Set up lookup cache
        $this->setupLookupOptions($this->user_level_permission_id);
        $this->setupLookupOptions($this->satker);
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->user_level_permission_id->CurrentValue = null;
        $this->user_level_permission_id->OldValue = $this->user_level_permission_id->CurrentValue;
        ...
        $this->satker->CurrentValue = null;
        $this->satker->OldValue = $this->satker->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'user_level_permission_id' first before field var 'x_user_level_permission_id'
        $val = $CurrentForm->hasValue("user_level_permission_id") ? $CurrentForm->getValue("user_level_permission_id") : $CurrentForm->getValue("x_user_level_permission_id");
        if (!$this->user_level_permission_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->user_level_permission_id->Visible = false; // Disable update for API request
            } else {
                $this->user_level_permission_id->setFormValue($val);
            }
        }
        ...
        // Check field name 'satker' first before field var 'x_satker'
        $val = $CurrentForm->hasValue("satker") ? $CurrentForm->getValue("satker") : $CurrentForm->getValue("x_satker");
        if (!$this->satker->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->satker->Visible = false; // Disable update for API request
            } else {
                $this->satker->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->user_level_permission_id->CurrentValue = $this->user_level_permission_id->FormValue;
        $this->_username->CurrentValue = $this->_username->FormValue;
        $this->_password->CurrentValue = $this->_password->FormValue;
        $this->nama_lengkap->CurrentValue = $this->nama_lengkap->FormValue;
        $this->satker->CurrentValue = $this->satker->FormValue;
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->user_level_permission_id->setDbValue($row['user_level_permission_id']);
        $this->_username->setDbValue($row['username']);
        $this->_password->setDbValue($row['password']);
        $this->nama_lengkap->setDbValue($row['nama_lengkap']);
        $this->satker->setDbValue($row['satker']);
    ...
    protected function newRow()
    {
        ...
        $row['user_level_permission_id'] = $this->user_level_permission_id->CurrentValue;
        ...
        $row['satker'] = $this->satker->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($Security->canAdmin()) { // System admin
                $curVal = trim(strval($this->user_level_permission_id->CurrentValue));
                if ($curVal != "") {
                    $this->user_level_permission_id->ViewValue = $this->user_level_permission_id->lookupCacheOption($curVal);
                    if ($this->user_level_permission_id->ViewValue === null) { // Lookup from database
                        $filterWrk = "`userlevelid`" . SearchString("=", $curVal, DATATYPE_NUMBER, "");
                        $sqlWrk = $this->user_level_permission_id->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                        $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                        $ari = count($rswrk);
                        if ($ari > 0) { // Lookup values found
                            $arwrk = $this->user_level_permission_id->Lookup->renderViewRow($rswrk[0]);
                            $this->user_level_permission_id->ViewValue = $this->user_level_permission_id->displayValue($arwrk);
                        } else {
                            $this->user_level_permission_id->ViewValue = $this->user_level_permission_id->CurrentValue;
                        }
                    }
                } else {
                    $this->user_level_permission_id->ViewValue = null;
                }
            } else {
                $this->user_level_permission_id->ViewValue = $Language->phrase("PasswordMask");
            }
            $this->user_level_permission_id->ViewCustomAttributes = "";
            ...
            // satker
            $curVal = trim(strval($this->satker->CurrentValue));
            if ($curVal != "") {
                $this->satker->ViewValue = $this->satker->lookupCacheOption($curVal);
                if ($this->satker->ViewValue === null) { // Lookup from database
                    $filterWrk = "`nama_satker`" . SearchString("=", $curVal, DATATYPE_STRING, "");
                    $sqlWrk = $this->satker->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                    $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                    $ari = count($rswrk);
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->satker->Lookup->renderViewRow($rswrk[0]);
                        $this->satker->ViewValue = $this->satker->displayValue($arwrk);
                    } else {
                        $this->satker->ViewValue = $this->satker->CurrentValue;
                    }
                }
            } else {
                $this->satker->ViewValue = null;
            }
            $this->satker->ViewCustomAttributes = "";
            ...
            // user_level_permission_id
            $this->user_level_permission_id->LinkCustomAttributes = "";
            $this->user_level_permission_id->HrefValue = "";
            $this->user_level_permission_id->TooltipValue = "";
            ...
            // satker
            $this->satker->LinkCustomAttributes = "";
            $this->satker->HrefValue = "";
            $this->satker->TooltipValue = "";
            ...
            // user_level_permission_id
            $this->user_level_permission_id->EditAttrs["class"] = "form-control";
            $this->user_level_permission_id->EditCustomAttributes = "";
            if (!$Security->canAdmin()) { // System admin
                $this->user_level_permission_id->EditValue = $Language->phrase("PasswordMask");
            } else {
                $curVal = trim(strval($this->user_level_permission_id->CurrentValue));
                if ($curVal != "") {
                    $this->user_level_permission_id->ViewValue = $this->user_level_permission_id->lookupCacheOption($curVal);
                } else {
                    $this->user_level_permission_id->ViewValue = $this->user_level_permission_id->Lookup !== null && is_array($this->user_level_permission_id->Lookup->Options) ? $curVal : null;
                }
                if ($this->user_level_permission_id->ViewValue !== null) { // Load from cache
                    $this->user_level_permission_id->EditValue = array_values($this->user_level_permission_id->Lookup->Options);
                } else { // Lookup from database
                    if ($curVal == "") {
                        $filterWrk = "0=1";
                    } else {
                        $filterWrk = "`userlevelid`" . SearchString("=", $this->user_level_permission_id->CurrentValue, DATATYPE_NUMBER, "");
                    }
                    $sqlWrk = $this->user_level_permission_id->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                    $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                    $ari = count($rswrk);
                    $arwrk = $rswrk;
                    $this->user_level_permission_id->EditValue = $arwrk;
                }
                $this->user_level_permission_id->PlaceHolder = RemoveHtml($this->user_level_permission_id->caption());
            }
            ...
            // satker
            $this->satker->EditAttrs["class"] = "form-control";
            $this->satker->EditCustomAttributes = "";
            $curVal = trim(strval($this->satker->CurrentValue));
            if ($curVal != "") {
                $this->satker->ViewValue = $this->satker->lookupCacheOption($curVal);
            } else {
                $this->satker->ViewValue = $this->satker->Lookup !== null && is_array($this->satker->Lookup->Options) ? $curVal : null;
            }
            if ($this->satker->ViewValue !== null) { // Load from cache
                $this->satker->EditValue = array_values($this->satker->Lookup->Options);
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "`nama_satker`" . SearchString("=", $this->satker->CurrentValue, DATATYPE_STRING, "");
                }
                $sqlWrk = $this->satker->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->satker->EditValue = $arwrk;
            }
            $this->satker->PlaceHolder = RemoveHtml($this->satker->caption());
    ...
    protected function validateForm()
    {
        ...
        if ($this->user_level_permission_id->Required) {
            if (!$this->user_level_permission_id->IsDetailKey && EmptyValue($this->user_level_permission_id->FormValue)) {
                $this->user_level_permission_id->addErrorMessage(str_replace("%s", $this->user_level_permission_id->caption(), $this->user_level_permission_id->RequiredErrorMessage));
            }
        }
        ...
        if ($this->satker->Required) {
            if (!$this->satker->IsDetailKey && EmptyValue($this->satker->FormValue)) {
                $this->satker->addErrorMessage(str_replace("%s", $this->satker->caption(), $this->satker->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // user_level_permission_id
        if ($Security->canAdmin()) { // System admin
            $this->user_level_permission_id->setDbValueDef($rsnew, $this->user_level_permission_id->CurrentValue, null, false);
        }
        ...
        // satker
        $this->satker->setDbValueDef($rsnew, $this->satker->CurrentValue, null, false);
    ...
    public function setupLookupOptions($fld)
    {
        if ($fld->Lookup !== null && $fld->Lookup->Options === null) {
            ...
            switch ($fld->FieldVar) {
                case "x_user_level_permission_id":
                    break;
                case "x_satker":
                    break;
                default:
