 ...
 * Page class
 */
class RequestDataGrid extends RequestData
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'request_data';

    // Page object name
    public $PageObjName = "RequestDataGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "frequest_datagrid";
    ...
    public function __construct()
    {
        ...
        // Table object (request_data)
        if (!isset($GLOBALS["request_data"]) || get_class($GLOBALS["request_data"]) == PROJECT_NAMESPACE . "request_data") {
            $GLOBALS["request_data"] = &$this;
        }

        // Page URL
        $pageUrl = $this->pageUrl();
        $this->AddUrl = "RequestDataAdd";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'request_data');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("request_data"));
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->request_data_id->setVisibility();
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->request_data_id->CurrentValue;
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_request_data_id") && $CurrentForm->hasValue("o_request_data_id") && $this->request_data_id->CurrentValue != $this->request_data_id->OldValue) {
            return false;
        }
    ...
    {
        $this->request_data_id->clearErrorMessage();
    ...
    {
        $this->request_data_id->CurrentValue = null;
        $this->request_data_id->OldValue = $this->request_data_id->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'request_data_id' first before field var 'x_request_data_id'
        $val = $CurrentForm->hasValue("request_data_id") ? $CurrentForm->getValue("request_data_id") : $CurrentForm->getValue("x_request_data_id");
        if (!$this->request_data_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->request_data_id->Visible = false; // Disable update for API request
            } else {
                $this->request_data_id->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_request_data_id")) {
            $this->request_data_id->setOldValue($CurrentForm->getValue("o_request_data_id"));
        }
    ...
    {
        global $CurrentForm;
        $this->request_data_id->CurrentValue = $this->request_data_id->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row = [];
        $row['request_data_id'] = $this->request_data_id->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->request_data_id->EditValue = HtmlEncode($this->request_data_id->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->request_data_id->Required) {
            if (!$this->request_data_id->IsDetailKey && EmptyValue($this->request_data_id->FormValue)) {
                $this->request_data_id->addErrorMessage(str_replace("%s", $this->request_data_id->caption(), $this->request_data_id->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->request_data_id->FormValue)) {
            $this->request_data_id->addErrorMessage($this->request_data_id->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // request_data_id
            $this->request_data_id->setDbValueDef($rsnew, $this->request_data_id->CurrentValue, 0, $this->request_data_id->ReadOnly);
    ...
    protected function addRow($rsold = null)
    {
        ...
        // Check referential integrity for master table 'request_data'
        $validMasterRecord = true;
        $masterFilter = $this->sqlMasterFilter_request2();
        ...
        // request_data_id
        $this->request_data_id->setDbValueDef($rsnew, $this->request_data_id->CurrentValue, 0, false);
        ...
        if ($insertRow && $this->ValidateKey && strval($rsnew['request_data_id']) == "") {
