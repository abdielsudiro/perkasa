 ...
 * Page class
 */
class RequestDataEdit extends RequestData
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RequestDataEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("request_data_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->request_data_id->setQueryStringValue($keyValue);
                $this->request_data_id->setOldValue($this->request_data_id->QueryStringValue);
            } elseif (Post("request_data_id") !== null) {
                $this->request_data_id->setFormValue(Post("request_data_id"));
                $this->request_data_id->setOldValue($this->request_data_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("request_data_id") ?? Route("request_data_id")) !== null) {
                    $this->request_data_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("RequestDataList"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "RequestDataList") {
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("RequestDataList"), "", $this->TableVar, true);
        $pageId = "edit";
