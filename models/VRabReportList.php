 ...
 * Page class
 */
class VRabReportList extends VRabReport
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'v_rab_report';

    // Page object name
    public $PageObjName = "VRabReportList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fv_rab_reportlist";
    ...
    public function __construct()
    {
        ...
        // Table object (v_rab_report)
        if (!isset($GLOBALS["v_rab_report"]) || get_class($GLOBALS["v_rab_report"]) == PROJECT_NAMESPACE . "v_rab_report") {
            $GLOBALS["v_rab_report"] = &$this;
        }
        ...
        $this->AddUrl = "VRabReportAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "VRabReportDelete";
        $this->MultiUpdateUrl = "VRabReportUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'v_rab_report');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fv_rab_reportlistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("v_rab_report"));
                $doc->Text = @$content;
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->volum->AdvancedSearch->toJson(), ","); // Field volum
        $filterList = Concat($filterList, $this->satuan->AdvancedSearch->toJson(), ","); // Field satuan
        $filterList = Concat($filterList, $this->sbm->AdvancedSearch->toJson(), ","); // Field sbm
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fv_rab_reportlistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field volum
        $this->volum->AdvancedSearch->SearchValue = @$filter["x_volum"];
        $this->volum->AdvancedSearch->SearchOperator = @$filter["z_volum"];
        $this->volum->AdvancedSearch->SearchCondition = @$filter["v_volum"];
        $this->volum->AdvancedSearch->SearchValue2 = @$filter["y_volum"];
        $this->volum->AdvancedSearch->SearchOperator2 = @$filter["w_volum"];
        $this->volum->AdvancedSearch->save();
        ...
        // Field sbm
        $this->sbm->AdvancedSearch->SearchValue = @$filter["x_sbm"];
        $this->sbm->AdvancedSearch->SearchOperator = @$filter["z_sbm"];
        $this->sbm->AdvancedSearch->SearchCondition = @$filter["v_sbm"];
        $this->sbm->AdvancedSearch->SearchValue2 = @$filter["y_sbm"];
        $this->sbm->AdvancedSearch->SearchOperator2 = @$filter["w_sbm"];
        $this->sbm->AdvancedSearch->save();
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fv_rab_reportlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fv_rab_reportlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fv_rab_reportlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fv_rab_reportlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
