 ...
 * Page class
 */
class TransitionEdit extends Transition
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'transition';

    // Page object name
    public $PageObjName = "TransitionEdit";
    ...
    public function __construct()
    {
        ...
        // Table object (transition)
        if (!isset($GLOBALS["transition"]) || get_class($GLOBALS["transition"]) == PROJECT_NAMESPACE . "transition") {
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'transition');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("transition"));
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->next_state_id->setVisibility();
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("transition_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->transition_id->setQueryStringValue($keyValue);
                $this->transition_id->setOldValue($this->transition_id->QueryStringValue);
            } elseif (Post("transition_id") !== null) {
                $this->transition_id->setFormValue(Post("transition_id"));
                $this->transition_id->setOldValue($this->transition_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("transition_id") ?? Route("transition_id")) !== null) {
                    $this->transition_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("TransitionList"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "TransitionList") {
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_transition_id")) {
            $this->transition_id->setOldValue($CurrentForm->getValue("o_transition_id"));
        }
        ...
        // Check field name 'next_state_id' first before field var 'x_next_state_id'
        $val = $CurrentForm->hasValue("next_state_id") ? $CurrentForm->getValue("next_state_id") : $CurrentForm->getValue("x_next_state_id");
        if (!$this->next_state_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->next_state_id->Visible = false; // Disable update for API request
            } else {
                $this->next_state_id->setFormValue($val);
            }
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        $this->next_state_id->CurrentValue = $this->next_state_id->FormValue;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->next_state_id->EditValue = HtmlEncode($this->next_state_id->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->next_state_id->Required) {
            if (!$this->next_state_id->IsDetailKey && EmptyValue($this->next_state_id->FormValue)) {
                $this->next_state_id->addErrorMessage(str_replace("%s", $this->next_state_id->caption(), $this->next_state_id->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->next_state_id->FormValue)) {
            $this->next_state_id->addErrorMessage($this->next_state_id->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // transition_id
            $this->transition_id->setDbValueDef($rsnew, $this->transition_id->CurrentValue, 0, $this->transition_id->ReadOnly);
            ...
            // next_state_id
            $this->next_state_id->setDbValueDef($rsnew, $this->next_state_id->CurrentValue, null, $this->next_state_id->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("TransitionList"), "", $this->TableVar, true);
        $pageId = "edit";
