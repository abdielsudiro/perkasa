 ...
 * Page class
 */
class RabRincianGrid extends RabRincian
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'rab_rincian';

    // Page object name
    public $PageObjName = "RabRincianGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "frab_rinciangrid";
    ...
    public function __construct()
    {
        ...
        // Table object (rab_rincian)
        if (!isset($GLOBALS["rab_rincian"]) || get_class($GLOBALS["rab_rincian"]) == PROJECT_NAMESPACE . "rab_rincian") {
            $GLOBALS["rab_rincian"] = &$this;
        }

        // Page URL
        $pageUrl = $this->pageUrl();
        $this->AddUrl = "RabRincianAdd";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'rab_rincian');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("rab_rincian"));
                $doc->Text = @$content;
    ...
    protected function hideFieldsForAddEdit()
    {
        if ($this->isAdd() || $this->isCopy() || $this->isGridAdd()) {
            $this->rab_rincian_id->Visible = false;
        }
    }
    ...
    public function run()
    {
        ...
        $this->rab_rincian_id->setVisibility();
    ...
    {
        $this->sbm->FormValue = ""; // Clear form value
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->rab_rincian_id->CurrentValue;
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_uraian") && $CurrentForm->hasValue("o_uraian") && $this->uraian->CurrentValue != $this->uraian->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_volum") && $CurrentForm->hasValue("o_volum") && $this->volum->CurrentValue != $this->volum->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_satuan") && $CurrentForm->hasValue("o_satuan") && $this->satuan->CurrentValue != $this->satuan->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_sbm") && $CurrentForm->hasValue("o_sbm") && $this->sbm->CurrentValue != $this->sbm->OldValue) {
            return false;
        }
    ...
    {
        $this->rab_rincian_id->clearErrorMessage();
        $this->uraian->clearErrorMessage();
        $this->volum->clearErrorMessage();
        $this->satuan->clearErrorMessage();
        $this->sbm->clearErrorMessage();
        $this->kode_akun->clearErrorMessage();
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canEdit()) {
                if (IsMobile()) {
                    ...
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"rab_rincian\" data-caption=\"" . $editcaption . "\" href=\"#\" onclick=\"return ew.modalDialogShow({lnk:this,btn:'SaveBtn',url:'" . HtmlEncode(GetUrl($this->EditUrl)) . "'});\">" . $Language->phrase("EditLink") . "</a>";
                }
            } else {
                $opt->Body = "";
            }
    ...
    {
        $this->rab_rincian_id->CurrentValue = null;
        $this->rab_rincian_id->OldValue = $this->rab_rincian_id->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'rab_rincian_id' first before field var 'x_rab_rincian_id'
        $val = $CurrentForm->hasValue("rab_rincian_id") ? $CurrentForm->getValue("rab_rincian_id") : $CurrentForm->getValue("x_rab_rincian_id");
        if (!$this->rab_rincian_id->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
            $this->rab_rincian_id->setFormValue($val);
        }
        ...
        if ($CurrentForm->hasValue("o_uraian")) {
            $this->uraian->setOldValue($CurrentForm->getValue("o_uraian"));
        }
        ...
        if ($CurrentForm->hasValue("o_volum")) {
            $this->volum->setOldValue($CurrentForm->getValue("o_volum"));
        }
        ...
        if ($CurrentForm->hasValue("o_sbm")) {
            $this->sbm->setOldValue($CurrentForm->getValue("o_sbm"));
        }
    ...
    public function restoreFormValues()
    {
        ...
        if (!$this->isGridAdd() && !$this->isAdd()) {
            $this->rab_rincian_id->CurrentValue = $this->rab_rincian_id->FormValue;
        }
    ...
    protected function newRow()
    {
        ...
        $row = [];
        $row['rab_rincian_id'] = $this->rab_rincian_id->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if (strval($this->sbm->EditValue) != "" && is_numeric($this->sbm->EditValue)) {
                ...
                $this->sbm->OldValue = $this->sbm->EditValue;
            }
            ...
            if ($curVal != "") {
                ...
                $this->kode_akun->ViewValue = $this->kode_akun->Lookup !== null && is_array($this->kode_akun->Lookup->Options) ? $curVal : null;
            }
            if ($this->kode_akun->ViewValue !== null) { // Load from cache
                $this->kode_akun->EditValue = array_values($this->kode_akun->Lookup->Options);
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "`Kode`" . SearchString("=", $this->kode_akun->CurrentValue, DATATYPE_STRING, "");
                }
                $sqlWrk = $this->kode_akun->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->kode_akun->EditValue = $arwrk;
            }
    ...
    protected function validateForm()
    {
        ...
        if ($this->rab_rincian_id->Required) {
            if (!$this->rab_rincian_id->IsDetailKey && EmptyValue($this->rab_rincian_id->FormValue)) {
                $this->rab_rincian_id->addErrorMessage(str_replace("%s", $this->rab_rincian_id->caption(), $this->rab_rincian_id->RequiredErrorMessage));
            }
        }
        ...
        if (!CheckInteger($this->volum->FormValue)) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "rab") {
            $this->id_rab->CurrentValue = $this->id_rab->getSessionValue();
        }
        $conn = $this->getConnection();
