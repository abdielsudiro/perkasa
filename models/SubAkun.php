...
/**
 * Table class for sub_akun
 */
class SubAkun extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'sub_akun';
        $this->TableName = 'sub_akun';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`sub_akun`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // detail_id
        $this->detail_id = new DbField('sub_akun', 'sub_akun', 'x_detail_id', 'detail_id', '`detail_id`', '`detail_id`', 3, 11, -1, false, '`detail_id`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->detail_id->IsPrimaryKey = true; // Primary key field
        $this->detail_id->Nullable = false; // NOT NULL field
        $this->detail_id->Required = true; // Required field
        $this->detail_id->Sortable = true; // Allow sort
        $this->detail_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->detail_id->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->detail_id->Param, "CustomMsg");
        $this->Fields['detail_id'] = &$this->detail_id;

        // kode_akun
        $this->kode_akun = new DbField('sub_akun', 'sub_akun', 'x_kode_akun', 'kode_akun', '`kode_akun`', '`kode_akun`', 200, 20, -1, false, '`kode_akun`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // detail
        $this->detail = new DbField('sub_akun', 'sub_akun', 'x_detail', 'detail', '`detail`', '`detail`', 201, 65535, -1, false, '`detail`', false, false, false, 'FORMATTED TEXT', 'TEXTAREA');
        ...
        // volum
        $this->volum = new DbField('sub_akun', 'sub_akun', 'x_volum', 'volum', '`volum`', '`volum`', 131, 10, -1, false, '`volum`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // sbm
        $this->sbm = new DbField('sub_akun', 'sub_akun', 'x_sbm', 'sbm', '`sbm`', '`sbm`', 5, 10, -1, false, '`sbm`', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`sub_akun`";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("SubAkunList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "SubAkunEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "SubAkunAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("SubAkunView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("SubAkunView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "SubAkunAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "SubAkunAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("SubAkunEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("SubAkunAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("SubAkunDelete", $this->getUrlParm());
    }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->detail_id);
        ...
        while (!$recordset->EOF && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->detail_id);
