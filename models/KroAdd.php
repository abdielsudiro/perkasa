...
class KroAdd extends Kro
{
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("KroList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "KroList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "KroView") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->nama_kro->CurrentValue = null;
        $this->nama_kro->OldValue = $this->nama_kro->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'nama_kro' first before field var 'x_nama_kro'
        $val = $CurrentForm->hasValue("nama_kro") ? $CurrentForm->getValue("nama_kro") : $CurrentForm->getValue("x_nama_kro");
        if (!$this->nama_kro->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->nama_kro->Visible = false; // Disable update for API request
            } else {
                $this->nama_kro->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->nama_kro->CurrentValue = $this->nama_kro->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row['nama_kro'] = $this->nama_kro->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->kode_kro->EditValue = HtmlEncode($this->kode_kro->CurrentValue);
            ...
            if ($this->kode_kegiatan->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = "`kode_kro`" . SearchString("=", $this->kode_kegiatan->CurrentValue, DATATYPE_STRING, "");
                }
            ...
            $this->nama_kro->EditValue = HtmlEncode($this->nama_kro->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->nama_kro->Required) {
            if (!$this->nama_kro->IsDetailKey && EmptyValue($this->nama_kro->FormValue)) {
                $this->nama_kro->addErrorMessage(str_replace("%s", $this->nama_kro->caption(), $this->nama_kro->RequiredErrorMessage));
            }
        }
        if ($this->id->Required) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        // kode_kro
        $this->kode_kro->setDbValueDef($rsnew, $this->kode_kro->CurrentValue, "", false);

        // kode_kegiatan
        $this->kode_kegiatan->setDbValueDef($rsnew, $this->kode_kegiatan->CurrentValue, null, false);

        // nama_kro
        $this->nama_kro->setDbValueDef($rsnew, $this->nama_kro->CurrentValue, null, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("KroList"), "", $this->TableVar, true);
