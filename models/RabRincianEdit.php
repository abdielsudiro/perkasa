 ...
 * Page class
 */
class RabRincianEdit extends RabRincian
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RabRincianEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("rab_rincian_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->rab_rincian_id->setQueryStringValue($keyValue);
                $this->rab_rincian_id->setOldValue($this->rab_rincian_id->QueryStringValue);
            } elseif (Post("rab_rincian_id") !== null) {
                $this->rab_rincian_id->setFormValue(Post("rab_rincian_id"));
                $this->rab_rincian_id->setOldValue($this->rab_rincian_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("rab_rincian_id") ?? Route("rab_rincian_id")) !== null) {
                    $this->rab_rincian_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("RabRincianList"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "RabRincianList") {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->rab_rincian_id->IsDetailKey) {
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("RabRincianList"), "", $this->TableVar, true);
        $pageId = "edit";
