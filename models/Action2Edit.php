 ...
 * Page class
 */
class Action2Edit extends Action2
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "Action2Edit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("action_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->action_id->setQueryStringValue($keyValue);
                $this->action_id->setOldValue($this->action_id->QueryStringValue);
            } elseif (Post("action_id") !== null) {
                $this->action_id->setFormValue(Post("action_id"));
                $this->action_id->setOldValue($this->action_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("action_id") ?? Route("action_id")) !== null) {
                    $this->action_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("Action2List"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "Action2List") {
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // action_id
            $this->action_id->setDbValueDef($rsnew, $this->action_id->CurrentValue, 0, $this->action_id->ReadOnly);

            // action_type_id
            $this->action_type_id->setDbValueDef($rsnew, $this->action_type_id->CurrentValue, null, $this->action_type_id->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("Action2List"), "", $this->TableVar, true);
        $pageId = "edit";
