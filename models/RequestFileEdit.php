 ...
 * Page class
 */
class RequestFileEdit extends RequestFile
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RequestFileEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("request_file_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->request_file_id->setQueryStringValue($keyValue);
                $this->request_file_id->setOldValue($this->request_file_id->QueryStringValue);
            } elseif (Post("request_file_id") !== null) {
                $this->request_file_id->setFormValue(Post("request_file_id"));
                $this->request_file_id->setOldValue($this->request_file_id->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("request_file_id") ?? Route("request_file_id")) !== null) {
                    $this->request_file_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("RequestFileList"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "RequestFileList") {
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("RequestFileList"), "", $this->TableVar, true);
        $pageId = "edit";
