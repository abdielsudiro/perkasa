 ...
 * Page class
 */
class ReportRabStatusSummary extends ReportRabStatus
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'Report RAB Status';

    // Page object name
    public $PageObjName = "ReportRabStatusSummary";
    ...
    public function __construct()
    {
        ...
        // Table object (ReportRABStatus)
        if (!isset($GLOBALS["ReportRABStatus"]) || get_class($GLOBALS["ReportRABStatus"]) == PROJECT_NAMESPACE . "ReportRABStatus") {
            $GLOBALS["ReportRABStatus"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'Report RAB Status');
        }
    ...
    public function loadGroupRowValues()
    {
        ...
        if ($this->GroupCount < $cnt) {
            $this->id_statuses->setGroupValue($this->GroupRecords[$this->GroupCount][0]);
        } else {
            $this->id_statuses->setGroupValue("");
        }
    }
    ...
    public function loadRowValues($record)
    {
        ...
        $data["id_statuses"] = $record['id_statuses'];
        $this->Rows[] = $data;
        $this->nama_satker->setDbValue($record['nama_satker']);
        $this->subtotal->setDbValue($record['subtotal']);
        $this->id_statuses->setDbValue(GroupValue($this->id_statuses, $record['id_statuses']));
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_SEARCH) { // Search row
            ...
            if ($this->RowTotalType == ROWTOTAL_GROUP) {
                $this->RowAttrs["data-group"] = $this->id_statuses->groupValue(); // Set up group attribute
            }
            if ($this->RowTotalType == ROWTOTAL_GROUP && $this->RowGroupLevel >= 2) {
                $this->RowAttrs["data-group-2"] = $this->subtotal->groupValue(); // Set up group attribute 2
            }

            // id_statuses
            $this->id_statuses->GroupViewValue = $this->id_statuses->groupValue();
            $this->id_statuses->GroupViewValue = FormatNumber($this->id_statuses->GroupViewValue, 0, 0, 0, 0);
            $this->id_statuses->CellCssClass = ($this->RowGroupLevel == 1 ? "ew-rpt-grp-summary-1" : "ew-rpt-grp-field-1");
            $this->id_statuses->ViewCustomAttributes = "";
            $this->id_statuses->GroupViewValue = DisplayGroupValue($this->id_statuses, $this->id_statuses->GroupViewValue);

            // subtotal
            $this->subtotal->GroupViewValue = $this->subtotal->groupValue();
            $this->subtotal->GroupViewValue = FormatNumber($this->subtotal->GroupViewValue, 2, -2, -2, -2);
            $this->subtotal->CellCssClass = ($this->RowGroupLevel == 2 ? "ew-rpt-grp-summary-2" : "ew-rpt-grp-field-2");
            $this->subtotal->CellCssStyle .= "text-align: right;";
            $this->subtotal->ViewCustomAttributes = "";
            $this->subtotal->GroupViewValue = DisplayGroupValue($this->subtotal, $this->subtotal->GroupViewValue);
            ...
            $this->id_statuses->GroupViewValue = $this->id_statuses->groupValue();
            $this->id_statuses->GroupViewValue = FormatNumber($this->id_statuses->GroupViewValue, 0, 0, 0, 0);
            $this->id_statuses->CellCssClass = "ew-rpt-grp-field-1";
            $this->id_statuses->ViewCustomAttributes = "";
            $this->id_statuses->GroupViewValue = DisplayGroupValue($this->id_statuses, $this->id_statuses->GroupViewValue);
            if (!$this->id_statuses->LevelBreak) {
                $this->id_statuses->GroupViewValue = "&nbsp;";
            } else {
                $this->id_statuses->LevelBreak = false;
            }

            // subtotal
            $this->subtotal->GroupViewValue = $this->subtotal->groupValue();
            $this->subtotal->GroupViewValue = FormatNumber($this->subtotal->GroupViewValue, 2, -2, -2, -2);
            $this->subtotal->CellCssClass = "ew-rpt-grp-field-2";
            $this->subtotal->CellCssStyle .= "text-align: right;";
            $this->subtotal->ViewCustomAttributes = "";
            $this->subtotal->GroupViewValue = DisplayGroupValue($this->subtotal, $this->subtotal->GroupViewValue);
            if (!$this->subtotal->LevelBreak) {
                $this->subtotal->GroupViewValue = "&nbsp;";
            } else {
                $this->subtotal->LevelBreak = false;
            }
        ...
        if ($this->RowType == ROWTYPE_TOTAL) {
            // id_statuses
            $currentValue = $this->id_statuses->GroupViewValue;
            $viewValue = &$this->id_statuses->GroupViewValue;
            $viewAttrs = &$this->id_statuses->ViewAttrs;
            $cellAttrs = &$this->id_statuses->CellAttrs;
            $hrefValue = &$this->id_statuses->HrefValue;
            $linkAttrs = &$this->id_statuses->LinkAttrs;
            $this->cellRendered($this->id_statuses, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);

            // subtotal
            $currentValue = $this->subtotal->GroupViewValue;
            $viewValue = &$this->subtotal->GroupViewValue;
            ...
            // id_statuses
            $currentValue = $this->id_statuses->groupValue();
            $viewValue = &$this->id_statuses->GroupViewValue;
            $viewAttrs = &$this->id_statuses->ViewAttrs;
            $cellAttrs = &$this->id_statuses->CellAttrs;
            $hrefValue = &$this->id_statuses->HrefValue;
            ...
            // subtotal
            $currentValue = $this->subtotal->groupValue();
            $viewValue = &$this->subtotal->GroupViewValue;
            $viewAttrs = &$this->subtotal->ViewAttrs;
            $cellAttrs = &$this->subtotal->CellAttrs;
            $hrefValue = &$this->subtotal->HrefValue;
