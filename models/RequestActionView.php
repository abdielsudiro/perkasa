 ...
 * Page class
 */
class RequestActionView extends RequestAction
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RequestActionView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("request_action_id") ?? Route("request_action_id")) !== null) {
            $this->RecKey["request_action_id"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("request_action_id") ?? Route("request_action_id")) !== null) {
                ...
                $this->RecKey["request_action_id"] = $this->request_action_id->QueryStringValue;
            } elseif (Post("request_action_id") !== null) {
                $this->request_action_id->setFormValue(Post("request_action_id"));
                $this->RecKey["request_action_id"] = $this->request_action_id->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->request_action_id->setQueryStringValue($keyValue);
                $this->RecKey["request_action_id"] = $this->request_action_id->QueryStringValue;
            } else {
                $returnUrl = "RequestActionList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "RequestActionList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "RequestActionList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
