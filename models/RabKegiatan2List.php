 ...
 * Page class
 */
class RabKegiatan2List extends RabKegiatan2
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RabKegiatan2List";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "frab_kegiatan2list";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "RabKegiatan2Add";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "RabKegiatan2Delete";
        $this->MultiUpdateUrl = "RabKegiatan2Update";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option frab_kegiatan2listsrch";
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->rab_kegiatan_id->AdvancedSearch->toJson(), ","); // Field rab_kegiatan_id
        ...
        $filterList = Concat($filterList, $this->total_biaya->AdvancedSearch->toJson(), ","); // Field total_biaya
        $filterList = Concat($filterList, $this->reviewer_note_id->AdvancedSearch->toJson(), ","); // Field reviewer_note_id
        $filterList = Concat($filterList, $this->approval_note_id->AdvancedSearch->toJson(), ","); // Field approval_note_id
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "frab_kegiatan2listsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field rab_kegiatan_id
        $this->rab_kegiatan_id->AdvancedSearch->SearchValue = @$filter["x_rab_kegiatan_id"];
        $this->rab_kegiatan_id->AdvancedSearch->SearchOperator = @$filter["z_rab_kegiatan_id"];
        $this->rab_kegiatan_id->AdvancedSearch->SearchCondition = @$filter["v_rab_kegiatan_id"];
        $this->rab_kegiatan_id->AdvancedSearch->SearchValue2 = @$filter["y_rab_kegiatan_id"];
        $this->rab_kegiatan_id->AdvancedSearch->SearchOperator2 = @$filter["w_rab_kegiatan_id"];
        $this->rab_kegiatan_id->AdvancedSearch->save();
        ...
        // Field reviewer_note_id
        $this->reviewer_note_id->AdvancedSearch->SearchValue = @$filter["x_reviewer_note_id"];
        $this->reviewer_note_id->AdvancedSearch->SearchOperator = @$filter["z_reviewer_note_id"];
        $this->reviewer_note_id->AdvancedSearch->SearchCondition = @$filter["v_reviewer_note_id"];
        $this->reviewer_note_id->AdvancedSearch->SearchValue2 = @$filter["y_reviewer_note_id"];
        $this->reviewer_note_id->AdvancedSearch->SearchOperator2 = @$filter["w_reviewer_note_id"];
        $this->reviewer_note_id->AdvancedSearch->save();

        // Field approval_note_id
        $this->approval_note_id->AdvancedSearch->SearchValue = @$filter["x_approval_note_id"];
        $this->approval_note_id->AdvancedSearch->SearchOperator = @$filter["z_approval_note_id"];
        $this->approval_note_id->AdvancedSearch->SearchCondition = @$filter["v_approval_note_id"];
        $this->approval_note_id->AdvancedSearch->SearchValue2 = @$filter["y_approval_note_id"];
        $this->approval_note_id->AdvancedSearch->SearchOperator2 = @$filter["w_approval_note_id"];
        $this->approval_note_id->AdvancedSearch->save();
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->kode_kro, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->kode_ro, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->kode_komponen, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->kode_subkomponen, $arKeywords, $type);
        return $where;
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->kode_kro); // kode_kro
            $this->updateSort($this->kode_ro); // kode_ro
            $this->updateSort($this->total_biaya); // total_biaya
            $this->updateSort($this->reviewer_note_id); // reviewer_note_id
            $this->updateSort($this->approval_note_id); // approval_note_id
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->reviewer_note_id->setSort("");
                $this->approval_note_id->setSort("");
            }
    ...
    public function renderListOptions()
    {
        ...
        $opt->Body = "<div class=\"custom-control custom-checkbox d-inline-block\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"custom-control-input ew-multi-select\" value=\"" . HtmlEncode($this->rab_kegiatan_id->CurrentValue) . "\" onclick=\"ew.clickMultiCheckbox(event);\"><label class=\"custom-control-label\" for=\"key_m_" . $this->RowCount . "\"></label></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"frab_kegiatan2listsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"frab_kegiatan2listsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.frab_kegiatan2list},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"frab_kegiatan2listsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
