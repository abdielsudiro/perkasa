 ...
 * Page class
 */
class RequestNoteGrid extends RequestNote
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'request_note';

    // Page object name
    public $PageObjName = "RequestNoteGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "frequest_notegrid";
    ...
    public function __construct()
    {
        ...
        // Table object (request_note)
        if (!isset($GLOBALS["request_note"]) || get_class($GLOBALS["request_note"]) == PROJECT_NAMESPACE . "request_note") {
            $GLOBALS["request_note"] = &$this;
        }

        // Page URL
        $pageUrl = $this->pageUrl();
        $this->AddUrl = "RequestNoteAdd";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'request_note');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("request_note"));
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->request_note_id->setVisibility();
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->request_note_id->CurrentValue;
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_request_note_id") && $CurrentForm->hasValue("o_request_note_id") && $this->request_note_id->CurrentValue != $this->request_note_id->OldValue) {
            return false;
        }
    ...
    {
        $this->request_note_id->clearErrorMessage();
    ...
    {
        $this->request_note_id->CurrentValue = null;
        $this->request_note_id->OldValue = $this->request_note_id->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'request_note_id' first before field var 'x_request_note_id'
        $val = $CurrentForm->hasValue("request_note_id") ? $CurrentForm->getValue("request_note_id") : $CurrentForm->getValue("x_request_note_id");
        if (!$this->request_note_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->request_note_id->Visible = false; // Disable update for API request
            } else {
                $this->request_note_id->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_request_note_id")) {
            $this->request_note_id->setOldValue($CurrentForm->getValue("o_request_note_id"));
        }
    ...
    {
        global $CurrentForm;
        $this->request_note_id->CurrentValue = $this->request_note_id->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row = [];
        $row['request_note_id'] = $this->request_note_id->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->request_note_id->EditValue = HtmlEncode($this->request_note_id->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->request_note_id->Required) {
            if (!$this->request_note_id->IsDetailKey && EmptyValue($this->request_note_id->FormValue)) {
                $this->request_note_id->addErrorMessage(str_replace("%s", $this->request_note_id->caption(), $this->request_note_id->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->request_note_id->FormValue)) {
            $this->request_note_id->addErrorMessage($this->request_note_id->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // request_note_id
            $this->request_note_id->setDbValueDef($rsnew, $this->request_note_id->CurrentValue, 0, $this->request_note_id->ReadOnly);
    ...
    protected function addRow($rsold = null)
    {
        ...
        // Check referential integrity for master table 'request_note'
        $validMasterRecord = true;
        $masterFilter = $this->sqlMasterFilter_request2();
        ...
        // request_note_id
        $this->request_note_id->setDbValueDef($rsnew, $this->request_note_id->CurrentValue, 0, false);
        ...
        if ($insertRow && $this->ValidateKey && strval($rsnew['request_note_id']) == "") {
