 ...
 * Page class
 */
class RequestFileView extends RequestFile
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RequestFileView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("request_file_id") ?? Route("request_file_id")) !== null) {
            $this->RecKey["request_file_id"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("request_file_id") ?? Route("request_file_id")) !== null) {
                ...
                $this->RecKey["request_file_id"] = $this->request_file_id->QueryStringValue;
            } elseif (Post("request_file_id") !== null) {
                $this->request_file_id->setFormValue(Post("request_file_id"));
                $this->RecKey["request_file_id"] = $this->request_file_id->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->request_file_id->setQueryStringValue($keyValue);
                $this->RecKey["request_file_id"] = $this->request_file_id->QueryStringValue;
            } else {
                $returnUrl = "RequestFileList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "RequestFileList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "RequestFileList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
