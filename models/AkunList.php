...
class AkunList extends Akun
{
    ...
    // Page object name
    public $PageObjName = "AkunList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fakunlist";
    ...
    public function __construct()
    {
        ...
        // Table object (akun)
        if (!isset($GLOBALS["akun"]) || get_class($GLOBALS["akun"]) == PROJECT_NAMESPACE . "akun") {
            $GLOBALS["akun"] = &$this;
        }
        ...
        $this->MultiDeleteUrl = "AkunDelete";
        $this->MultiUpdateUrl = "AkunUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'akun');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fakunlistsrch";
    ...
    public function run()
    {
        ...
        $this->isSubAccount->Visible = false;
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->isSubAccount->AdvancedSearch->toJson(), ","); // Field isSubAccount
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function restoreFilterList()
    {
        ...
        // Field isSubAccount
        $this->isSubAccount->AdvancedSearch->SearchValue = @$filter["x_isSubAccount"];
        $this->isSubAccount->AdvancedSearch->SearchOperator = @$filter["z_isSubAccount"];
        $this->isSubAccount->AdvancedSearch->SearchCondition = @$filter["v_isSubAccount"];
        $this->isSubAccount->AdvancedSearch->SearchValue2 = @$filter["y_isSubAccount"];
        $this->isSubAccount->AdvancedSearch->SearchOperator2 = @$filter["w_isSubAccount"];
        $this->isSubAccount->AdvancedSearch->save();
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canEdit()) {
                if (IsMobile()) {
                    ...
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"akun\" data-caption=\"" . $editcaption . "\" href=\"#\" onclick=\"return ew.modalDialogShow({lnk:this,btn:'SaveBtn',url:'" . HtmlEncode(GetUrl($this->EditUrl)) . "'});\">" . $Language->phrase("EditLink") . "</a>";
                }
            } else {
                $opt->Body = "";
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fakunlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fakunlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fakunlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fakunlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
