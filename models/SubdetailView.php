 ...
 * Page class
 */
class SubdetailView extends Subdetail
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SubdetailView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("subdetail_id") ?? Route("subdetail_id")) !== null) {
            $this->RecKey["subdetail_id"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("subdetail_id") ?? Route("subdetail_id")) !== null) {
                ...
                $this->RecKey["subdetail_id"] = $this->subdetail_id->QueryStringValue;
            } elseif (Post("subdetail_id") !== null) {
                $this->subdetail_id->setFormValue(Post("subdetail_id"));
                $this->RecKey["subdetail_id"] = $this->subdetail_id->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->subdetail_id->setQueryStringValue($keyValue);
                $this->RecKey["subdetail_id"] = $this->subdetail_id->QueryStringValue;
            } else {
                $returnUrl = "SubdetailList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "SubdetailList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "SubdetailList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
