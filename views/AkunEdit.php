...
loadjs.ready("head", function () {
    ...
    fakunedit = currentForm = new ew.Form("fakunedit", "edit");
    ...
    // Form_CustomValidate
    fakunedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fakunedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fakunedit.lists.parent_id = <?= $Page->parent_id->toClientList($Page) ?>;
    fakunedit.lists.isSubAccount = <?= $Page->isSubAccount->toClientList($Page) ?>;
    loadjs.done("fakunedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fakunedit" id="fakunedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="akun" data-field="x_id" data-hidden="1" name="o_id" id="o_id" value="<?= HtmlEncode($Page->id->OldValue ?? $Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->isSubAccount->isInvalidClass() ?>" data-table="akun" data-field="x_isSubAccount" name="x_isSubAccount[]" id="x_isSubAccount_587729" value="1"<?= ConvertToBool($Page->isSubAccount->CurrentValue) ? " checked" : "" ?><?= $Page->isSubAccount->editAttributes() ?> aria-describedby="x_isSubAccount_help">
    <label class="custom-control-label" for="x_isSubAccount_587729"></label>
</div>
