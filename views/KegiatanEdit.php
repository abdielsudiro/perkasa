...
loadjs.ready("head", function () {
    ...
    fkegiatanedit = currentForm = new ew.Form("fkegiatanedit", "edit");
    ...
    // Validate form
    fkegiatanedit.validate = function () {
    ...
    fkegiatanedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fkegiatanedit.lists.kode_program = <?= $Page->kode_program->toClientList($Page) ?>;
...
<input type="hidden" data-table="kegiatan" data-field="x_id" data-hidden="1" name="o_id" id="o_id" value="<?= HtmlEncode($Page->id->OldValue ?? $Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
</div><!-- /page* -->
