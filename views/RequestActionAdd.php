...
// Page object
$RequestActionAdd = &$Page;
?>
<script>
var currentForm, currentPageID;
var frequest_actionadd;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "add";
    frequest_actionadd = currentForm = new ew.Form("frequest_actionadd", "add");
    ...
    // Validate form
    frequest_actionadd.validate = function () {
    ...
    // Form_CustomValidate
    frequest_actionadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    frequest_actionadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    frequest_actionadd.lists.is_active = <?= $Page->is_active->toClientList($Page) ?>;
    frequest_actionadd.lists.is_complete = <?= $Page->is_complete->toClientList($Page) ?>;
    loadjs.done("frequest_actionadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="frequest_actionadd" id="frequest_actionadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->is_active->isInvalidClass() ?>" data-table="request_action" data-field="x_is_active" name="x_is_active[]" id="x_is_active_858028" value="1"<?= ConvertToBool($Page->is_active->CurrentValue) ? " checked" : "" ?><?= $Page->is_active->editAttributes() ?> aria-describedby="x_is_active_help">
    <label class="custom-control-label" for="x_is_active_858028"></label>
</div>
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->is_complete->isInvalidClass() ?>" data-table="request_action" data-field="x_is_complete" name="x_is_complete[]" id="x_is_complete_530581" value="1"<?= ConvertToBool($Page->is_complete->CurrentValue) ? " checked" : "" ?><?= $Page->is_complete->editAttributes() ?> aria-describedby="x_is_complete_help">
    <label class="custom-control-label" for="x_is_complete_530581"></label>
</div>
