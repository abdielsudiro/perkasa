...
loadjs.ready("head", function () {
    ...
    froedit = currentForm = new ew.Form("froedit", "edit");
    ...
    // Form_CustomValidate
    froedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    froedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    froedit.lists.kode_kro = <?= $Page->kode_kro->toClientList($Page) ?>;
    loadjs.done("froedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="froedit" id="froedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="ro" data-field="x_id" data-hidden="1" name="o_id" id="o_id" value="<?= HtmlEncode($Page->id->OldValue ?? $Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
</div><!-- /page* -->
