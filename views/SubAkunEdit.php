...
loadjs.ready("head", function () {
    ...
    fsub_akunedit = currentForm = new ew.Form("fsub_akunedit", "edit");
    ...
    // Validate form
    fsub_akunedit.validate = function () {
    ...
    // Form_CustomValidate
    fsub_akunedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fsub_akunedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fsub_akunedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fsub_akunedit" id="fsub_akunedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="sub_akun" data-field="x_detail_id" data-hidden="1" name="o_detail_id" id="o_detail_id" value="<?= HtmlEncode($Page->detail_id->OldValue ?? $Page->detail_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
