...
// Page object
$ActivityTypeEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var factivity_typeedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    factivity_typeedit = currentForm = new ew.Form("factivity_typeedit", "edit");
    ...
    // Validate form
    factivity_typeedit.validate = function () {
    ...
    // Form_CustomValidate
    factivity_typeedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    factivity_typeedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("factivity_typeedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="factivity_typeedit" id="factivity_typeedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="activity_type" data-field="x_activity_type_id" data-hidden="1" name="o_activity_type_id" id="o_activity_type_id" value="<?= HtmlEncode($Page->activity_type_id->OldValue ?? $Page->activity_type_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
