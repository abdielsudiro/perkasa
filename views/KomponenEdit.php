...
loadjs.ready("head", function () {
    ...
    fkomponenedit = currentForm = new ew.Form("fkomponenedit", "edit");
    ...
    // Validate form
    fkomponenedit.validate = function () {
    ...
    // Form_CustomValidate
    fkomponenedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fkomponenedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fkomponenedit.lists.kode_ro = <?= $Page->kode_ro->toClientList($Page) ?>;
    loadjs.done("fkomponenedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fkomponenedit" id="fkomponenedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="komponen" data-field="x_id" data-hidden="1" name="o_id" id="o_id" value="<?= HtmlEncode($Page->id->OldValue ?? $Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
</div><!-- /page* -->
