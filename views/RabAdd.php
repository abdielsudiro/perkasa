...
loadjs.ready("head", function () {
    ...
    frabadd = currentForm = new ew.Form("frabadd", "add");
    ...
    // Form_CustomValidate
    frabadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    frabadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    frabadd.lists.satker_id = <?= $Page->satker_id->toClientList($Page) ?>;
    frabadd.lists.kode_program = <?= $Page->kode_program->toClientList($Page) ?>;
    frabadd.lists.kode_kegiatan = <?= $Page->kode_kegiatan->toClientList($Page) ?>;
    frabadd.lists.kode_kro = <?= $Page->kode_kro->toClientList($Page) ?>;
    frabadd.lists.kode_komponen = <?= $Page->kode_komponen->toClientList($Page) ?>;
    frabadd.lists.kode_subkomponen = <?= $Page->kode_subkomponen->toClientList($Page) ?>;
    frabadd.lists.kode_ro = <?= $Page->kode_ro->toClientList($Page) ?>;
    loadjs.done("frabadd");
});
</script>
<script>
loadjs.ready("head", function () {
    // Client script
    $((function(){var e=$("input:radio[name=status]");!1===e.is(":checked")&&e.filter("[value=menunggu]").prop("checked",!0)}));
});
</script>
...
$Page->showMessage();
?>
<form name="frabadd" id="frabadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php
    if (in_array("rab_file", explode(",", $Page->getCurrentDetailTable())) && $rab_file->DetailAdd) {
?>
...
<?php } ?>
<?php
    if (in_array("rab_note_penelaah", explode(",", $Page->getCurrentDetailTable())) && $rab_note_penelaah->DetailAdd) {
?>
<?php if ($Page->getCurrentDetailTable() != "") { ?>
...
<?php } ?>
<?php
    if (in_array("rab_note_penyetuju", explode(",", $Page->getCurrentDetailTable())) && $rab_note_penyetuju->DetailAdd) {
?>
<?php if ($Page->getCurrentDetailTable() != "") { ?>
...
<?php } ?>
<?php
    if (in_array("rab_rincian", explode(",", $Page->getCurrentDetailTable())) && $rab_rincian->DetailAdd) {
?>
<?php if ($Page->getCurrentDetailTable() != "") { ?>
