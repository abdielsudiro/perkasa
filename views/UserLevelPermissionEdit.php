...
// Page object
$UserLevelPermissionEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fuser_level_permissionedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fuser_level_permissionedit = currentForm = new ew.Form("fuser_level_permissionedit", "edit");
    ...
    fuser_level_permissionedit.addFields([
    ...
    // Set invalid fields
    $(function() {
        var f = fuser_level_permissionedit,
            fobj = f.getForm(),
            $fobj = $(fobj),
    ...
    // Validate form
    fuser_level_permissionedit.validate = function () {
    ...
    // Form_CustomValidate
    fuser_level_permissionedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fuser_level_permissionedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fuser_level_permissionedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fuser_level_permissionedit" id="fuser_level_permissionedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->user_level_permission_id->Visible) { // user_level_permission_id ?>
    <div id="r_user_level_permission_id" class="form-group row">
        <label id="elh_user_level_permission_user_level_permission_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->user_level_permission_id->caption() ?><?= $Page->user_level_permission_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->user_level_permission_id->getDisplayValue($Page->user_level_permission_id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="user_level_permission" data-field="x_user_level_permission_id" data-hidden="1" name="x_user_level_permission_id" id="x_user_level_permission_id" value="<?= HtmlEncode($Page->user_level_permission_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
