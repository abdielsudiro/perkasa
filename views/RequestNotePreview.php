...
// Page object
$RequestNotePreview = &$Page;
?>
<?php $Page->showPageHeader(); ?>
<?php if ($Page->TotalRecords > 0) { ?>
<div class="card ew-grid request_note"><!-- .card -->
...
<?php if ($Page->request_note_id->Visible) { // request_note_id ?>
    <?php if ($Page->SortUrl($Page->request_note_id) == "") { ?>
        <th class="<?= $Page->request_note_id->headerCellClass() ?>"><?= $Page->request_note_id->caption() ?></th>
    <?php } else { ?>
        <th class="<?= $Page->request_note_id->headerCellClass() ?>"><div class="ew-pointer" data-sort="<?= HtmlEncode($Page->request_note_id->Name) ?>" data-sort-order="<?= $Page->SortField == $Page->request_note_id->Name && $Page->SortOrder == "ASC" ? "DESC" : "ASC" ?>">
            <div class="ew-table-header-btn"><span class="ew-table-header-caption"><?= $Page->request_note_id->caption() ?></span><span class="ew-table-header-sort"><?php if ($Page->SortField == $Page->request_note_id->Name) { ?><?php if ($Page->SortOrder == "ASC") { ?><i class="fas fa-sort-up"></i><?php } elseif ($Page->SortOrder == "DESC") { ?><i class="fas fa-sort-down"></i><?php } ?><?php } ?></span>
        </div></div></th>
    <?php } ?>
<?php } ?>
