...
// Page object
$RequestFileEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var frequest_fileedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    frequest_fileedit = currentForm = new ew.Form("frequest_fileedit", "edit");
    ...
    // Validate form
    frequest_fileedit.validate = function () {
    ...
    // Form_CustomValidate
    frequest_fileedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    frequest_fileedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("frequest_fileedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="frequest_fileedit" id="frequest_fileedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="request_file" data-field="x_request_file_id" data-hidden="1" name="o_request_file_id" id="o_request_file_id" value="<?= HtmlEncode($Page->request_file_id->OldValue ?? $Page->request_file_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["frequest_fileedit", "datetimepicker"], function() {
    ew.createDateTimePicker("frequest_fileedit", "x_date_uploaded", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
